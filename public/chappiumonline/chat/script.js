...
function wait() {
  ...
  if (auth.currentUser == null) {
    window.setTimeout(function(){wait()}, 500);
  } else {
    ...
    postsref.on("value", (snapshot) => {
      ...
      data.forEach((item) => {
        if (item != '') {
          ...
          li.innerHTML = '<div class="messageleft"><div class="imagediv"><img src="' + item.split('-->').at(3) +'" alt="Avatar"/></div><div class="message"><p>'+item.split('-->').at(1)+'</p><span class="time-right">'+'</span></div></div>'
          } else {
          li.innerHTML = '<div class="messageright"><div class="message darker"><p>'+item.split('-->').at(1)+'</p><span class="time-right">'+'</span></div><div class="imagediv"><img class="right" src="' + item.split('-->').at(3) +'" alt="Avatar"/></div></div>' }
      ...
      function sndBtnClicked() {
        if (document.getElementById("chat").value != '') {
          var postsref2 = database.ref("ChappUsers/" + user.email.split("@")[0].replaceAll(".","*"));
